...
func EmailVerificationGET(w http.ResponseWriter, r *http.Request) {
	...
	result, err := models.UserEmailVerified(token)
...
func RegisterPOST(w http.ResponseWriter, r *http.Request) {
	...
	// Get database result
	_, err := models.UserIdByEmail(email)

	if err == sql.ErrNoRows { // If success (no user exists with that email)
		result, ex := models.UserCreate(first_name, last_name, email, password)
		// Will only error if there is a problem with the query
		if ex != nil {
			log.Println(ex)
			sess.AddFlash(view.Flash{"An error occurred on the server. Please try again later.", view.FlashError})
			...
			// Add the user role
			models.RoleCreate(user_id, models.Role_level_User)

			// Add the hash to the database
			models.UserEmailVerificationCreate(user_id, md)
